...
import com.bzua.jingdata.dto.AssetPortfolio;
import com.bzua.jingdata.dto.ProductFileInformation;
import com.bzua.jingdata.util.Constant;
import com.bzua.jingdata.util.FileUtils;
import com.bzua.jingdata.util.JZFileUtil;
...
public class ExplainXSSF {
     ...
     * @return
     */
    private static List<String> getAllHeader(List<Field> fieldList) {
    ...
    private static XSSFWorkbook getWorkbook(List<?> dataList, Boolean isMerge) {
        ...
        for (int row = 0; row < dataList.size(); row++) {
            ...
            for (int i = 0; i < methodList.size(); i++) {
                ...
                try {
                    Object invoke = methodList.get(i).invoke(data);
                    if (invoke == null || invoke == "0" || invoke == "0.00") {
                        if (fieldList.get(i).getAnnotation(XSSF.class).isMoney() || fieldList.get(i).getAnnotation(XSSF.class).isRatio()) {
                            invoke = "0.00";
                        } else if (fieldList.get(i).getAnnotation(XSSF.class).isNumber()) {
                            invoke = "0";
                        }
                    }
                    if (fieldList.get(i).getAnnotation(XSSF.class).isRatio()) {
                        invoke += "%";
                    }
                    if (fieldList.get(i).getAnnotation(XSSF.class).isCNY()) {
                        invoke = "CNY " + invoke;
                    }
                    if (fieldList.get(i).getAnnotation(XSSF.class).header().contains("是否")) {
                        if (invoke != null) {
                            if ("1".equals(invoke)) {
                                invoke = "是";
                            } else {
                    ...
                    CellUtil.setCellValue(cell, invoke, cellStyleList.get(i));
                } catch (IllegalAccessException | InvocationTargetException e) {
                    log.error("{}", e);
                }
            }
    ...
    private static Class getClassByFirstDataFromList(List<?> dataList) {
        return getListClass(dataList);
    }

    private static List<Method> getAllMethod(List<?> dataList) {
        Class clazz = getClassByFirstDataFromList(dataList);
        List<Field> fieldList = getAllField(clazz);
    ...
    public static File enclosureFile(List<ProductFileInformation> dataList, String tokenUrl, String userCookie, String fileName, String startTime, String endTime, String numSettingUrl, Boolean isTxt) {
        ...
        try {
            ...
            if (isTxt) {
                originFile = ExplainXSSF.getTxtFile(dataList, fileName, startTime, endTime, numSettingUrl);
            } else {
                originFile = ExplainXSSF.getExcelFile(dataList, fileName, numSettingUrl, false, false);
            }
